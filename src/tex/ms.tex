...
\begin{document}
	...
	\begin{abstract}
		...
		achieves low relative integrated squared (Frobenius) error (RISE) that improves with
		sample size. We
		...
		LISA noise by an order of magnitude in relative integrated squared
		(Frobenius) error.
	\end{abstract}
	...
	factor $N_{b}N_{h}$. We treat $\eta$ as a tunable calibration
	...
	metrics. The first is the relative integrated squared (Frobenius) error
	(RISE),
	\begin{equation}
		\operatorname{RISE}= \frac{\int \! \left\lVert \widehat{S}(f) - S(f) \right\rVert_{F}^{2}
		\, \mathrm{d}f}{\int \! \left\lVert S(f) \right\rVert_{F}^{2} \, \mathrm{d}f},
	\end{equation}
	where $\lVert \cdot \rVert_{F}$ denotes the Frobenius norm and the
	integrals are evaluated numerically using Simpson's rule. RISE is an
	$L^{2}$-energy ratio, monotone-equivalent to the standard relative
	integrated absolute error (RIAE). This metric measures the bias of the
	...
	\begin{table*}[!htbp]
		...
		RISE is the relative integrated squared (Frobenius) error. ESS
	...
	the orthonormal rotation $(A, E, T)^{\top} = M_{\mathrm{AET}}\,(X, Y, Z)^{\top}$
	with
	\begin{equation}
		\label{eq:M_AET}
		M_{\mathrm{AET}} = \begin{pmatrix}
			1/\sqrt{2} & 0 & -1/\sqrt{2} \\
			1/\sqrt{6} & -2/\sqrt{6} & 1/\sqrt{6} \\
			1/\sqrt{3} & 1/\sqrt{3} & 1/\sqrt{3}
		\end{pmatrix},
	\end{equation}
	which satisfies $M_{\mathrm{AET}}^{\top}M_{\mathrm{AET}} = I$ and has unit
	...
	\begin{itemize}
		...
		rotated into AET via $M_{\mathrm{AET}}$ (Eq.~\ref{eq:M_AET}),
		...
		$\log S_{AA}, \log S_{EE}, \log S_{TT}$) with $K=100$ knots
		each. Only the log-diagonal expansion
		...
		Equivalently, this is the multivariate complex Wishart
		likelihood on the $3\times3$ averaged periodogram matrix
		$\Y_{h}^{*}$ with all off-diagonal Cholesky coefficients
		$\theta_{jl}$ fixed to zero, since for a diagonal $\S$ the
		Wishart likelihood factorises into the three univariate
		diagonal terms. The diagonal AET posterior is assembled
		per-sample as $\mathrm{diag}(S_{AA},S_{EE},S_{TT})$ and
		rotated back to the XYZ basis via
		$\S_{\mathrm{XYZ}}=M_{\mathrm{AET}}^{\top}\S_{\mathrm{AET}}M_{\mathrm{AET}}$
		...
		\emph{approximately} diagonal in the AET basis. The full
		multivariate Cholesky model
		(Eqs.~\ref{eq:delta_spline}--\ref{eq:theta_im_spline})
		is fit to the AET-rotated time series with $K=100$ knots
		on the diagonals $\log\delta_{j}^{2}$ but only $K_{\theta}=2$
		knots on the off-diagonal components $\theta_{jl}$. The narrow off-diagonal
		basis keeps $\S_{\mathrm{AET}}$ near diagonal while still
		allowing the data to push back if smooth off-diagonal
		structure is present. The posterior spectral matrix is
		...
		(Eqs.~\ref{eq:delta_spline}--\ref{eq:theta_im_spline})
		is fit directly to the XYZ time series with $K=100$ knots
		for \emph{every} Cholesky component, both diagonal and
	...
	\begin{table*}[!htbp]
		...
		\caption{LISA TDI XYZ results at fixed safe-Bayes
			...
			$1024$. RISE is the relative integrated squared (Frobenius)
