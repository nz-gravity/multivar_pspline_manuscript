...
\begin{document}
	...
	\citep{lisa_noise4a, lisa_noise5a}. The dataset was
	produced with \texttt{lisainstrument}~\citep{lisainstrument} under the
	LDC Spritz noise model~\citep{ldcspritz} at a sampling rate of
	...
	In addition to the RIAE introduced in Section~\ref{subsec:sim_3d_var2},
	...
	\end{equation}
	computed in the XYZ basis for both models. This complements the RIAE
	(a bias metric) with a summary of posterior precision.
	Table~\ref{tab:lisa_results} reports RIAE, $\Delta_{\mathrm{PSD}}$, and
	wall-clock duration for the two models, two datasets, and three
	observation durations. All twelve LISA runs satisfy the convergence
	diagnostics of
