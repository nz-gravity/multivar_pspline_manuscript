...
\begin{document}
    ...
    but realistic noise (unequal arm lengths, per-MOSA (Movable Optical Sub-Assembly) imbalance, and
	...
	\begin{equation}
		\Y_{h}^{*}\;\dot\sim\;\mathcal{CW}_{p}\!\left(T_b\,\S(\bar{f}_{h}),\, N_{b}N_{h}\right).
	\end{equation}
	...
	every configuration, and the mean RIAE varies by less than 3\% across
	...
	XYZ$\to$AET orthogonal transformation (the noise-orthogonal $A$, $E$, $T$ TDI combinations) diagonalises the LISA
	...
	datasets under the $\mathcal{H}_{XYZ}$ model. The
