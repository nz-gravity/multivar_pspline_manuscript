...
\begin{document}
	...
	\begin{equation}
		\label{eq:log-like}
		\mathcal{L}_{b}(\d|\S)
		\propto \prod_{k=1}^{n/(2N_{b})} \left|\S(f_{k})\right|^{-N_{b}}
	...
	\begin{figure*}[!t]
		\centering
		\resizebox{\linewidth}{!}{\input{blocked_likelihood_tikz}}
		\caption{Construction of the blocked, coarse-grained periodogram.
			...
			single-block periodograms each of length $n/(2N_{b})$. The
			block-averaged periodogram (Welch estimator) is then summed
			over consecutive groups of $N_{h}$ adjacent fine frequencies to
			produce the final coarse-grained periodogram of length
			$n/(2N_{b}N_{h})$.}
	...
	\end{equation}
	with eigendecomposition $\Y_{h}^{*}=\sum_{\nu=1}^{p}
	\u^{(h)}_{\nu}\u^{(h)*}_{\nu}$,
	$\u^{(h)}_{\nu}=\sqrt{\lambda^{(h)}_{\nu}}\,\v^{(h)}_{\nu}$.
	...
	\begin{align}
		...
		\Im[\theta_{jl}(f_{k})] & = \sum_{m=1}^{K_{jl}}B_{m}(f_{k}) w_{jl,m}^{(\Im)}\label{eq:theta_im_spline}
	\end{align}
	...
	\begin{figure}
		\centering
		\includegraphics[width=1\linewidth]{figures/vi_vs_nuts_var3.pdf}
	...
	follow Section~\ref{sec:computation}.
	...
	\begin{figure*}[!t]
		\centering
		\includegraphics[width=0.48\linewidth]{figures/triangle_noise4a_eta0p5.pdf}\hfill
		\includegraphics[width=0.48\linewidth]{figures/triangle_noise5a_eta0p5.pdf}
	...
	The datasets generated are available at the public Zenodo repository.
   ...
   ~\citet{Liu2023}. Entries are mean $\pm$ standard deviation over 500
	...
	\begin{figure}[!t]
		\centering
		\includegraphics[width=\linewidth]{figures/lisa_eta_sweep.pdf}
