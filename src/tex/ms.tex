...
\begin{document}
	...
	noise covariance to within statistical resolution. The three
	hypotheses form a strictly nested hierarchy of model flexibility,
	$\mathcal{H}_{0}\subset\mathcal{H}_{1}\subset\mathcal{H}_{2}$:
	\begin{itemize}
		\item $\mathcal{H}_{0}$ (\emph{univariate diagonal AET}) ---
		the cross-spectral matrix is taken to be \emph{exactly}
		diagonal in the $A,E,T$ basis. The XYZ time series is
		...
		correction), and each of the three channels is then fit
		\emph{independently} with a univariate ($p=1$) log-P-spline
		model on $\log S_{AA}, \log S_{EE}, \log S_{TT}$ ($K=100$
		knots each). With $p=1$ there are no off-diagonal Cholesky
		factors to sample, so the AET off-diagonals are
		zero-by-construction. The diagonal AET posterior is
		assembled per-sample as $\mathrm{diag}(S_{AA},S_{EE},S_{TT})$
		and rotated back to the XYZ basis via
		$\S_{XYZ}=M_{\mathrm{AET}}^{\top}\S_{AET}M_{\mathrm{AET}}$
		...
		\emph{approximately} diagonal in the $A,E,T$ basis. The
		full multivariate Cholesky model is fit to the AET-rotated
		time series with $K=100$ knots on the diagonals
		$\log\delta_{j}^{2}$ but only $K_{\theta}=2$ knots on the
		off-diagonal components $\theta_{jk}$. The narrow
		off-diagonal basis keeps $\S_{AET}$ near diagonal while
		still allowing the data to push back if smooth
		off-diagonal structure is present. The posterior spectral
		matrix is rotated back to the XYZ basis for diagnostics.
		\item $\mathcal{H}_{2}$ (\emph{full multivariate XYZ}) ---
		the cross-spectral matrix has fully populated off-diagonals.
		The multivariate Cholesky model is fit directly to the
		$X_{2}Y_{2}Z_{2}$ time series with $K=100$ knots for
		\emph{every} Cholesky component, diagonal and off-diagonal.
		This is the unrestricted reference model.
	\end{itemize}
	...
	models should agree to within statistical resolution. For
	...
	knots, and $\mathcal{H}_{2}$ remains unrestricted. The expected
	fit-quality ordering is therefore
	$\mathrm{RIAE}(\mathcal{H}_{0})\!>\!\mathrm{RIAE}(\mathcal{H}_{1})\!>\!\mathrm{RIAE}(\mathcal{H}_{2})$
	for \texttt{noise5a} and
	$\mathrm{RIAE}(\mathcal{H}_{0})\!\approx\!\mathrm{RIAE}(\mathcal{H}_{1})\!\approx\!\mathrm{RIAE}(\mathcal{H}_{2})$
	for \texttt{noise4a}.
	...
	\end{equation}
	computed in the XYZ basis for all three models. This complements the RIAE
	...
	divergent transitions or tree-depth saturation. The results split sharply
	...
	($\mathrm{RIAE}\approx 4.4\!-\!5.3\times10^{-3}$) across all
	...
	$M_{\mathrm{AET}}^{\top}\!\cdot\!M_{\mathrm{AET}}$ rotation, and
	...
	$\mathrm{RIAE}\approx 3.3\!\times\!10^{-2}$ at all durations ---
	a floor set by the AET off-diagonal power it is structurally
	unable to represent. $\mathcal{H}_{1}$ improves on this by
	roughly $50\%$
	($\mathrm{RIAE}\approx 1.8\!-\!2.2\!\times\!10^{-2}$) by
	allowing two off-diagonal knots, but is still limited
	by its narrow $\theta_{jk}$ basis. $\mathcal{H}_{2}$ reaches
	$\mathrm{RIAE}\approx 9\!-\!11\!\times\!10^{-4}$, about
	...
	the AET off-diagonals, as the order-of-magnitude RIAE gap to
	...
	\begin{table*}[!htbp]
		...
		\caption{LISA TDI $X_{2}Y_{2}Z_{2}$ results at fixed safe-Bayes
			...
			univariate diagonal-AET null model ($\mathcal{H}_{0}$,
			three independent $p=1$ fits), the AET-rotated multivariate
			...
			rotated back to XYZ via $M_{\mathrm{AET}}$).
		...
		\Body
			...
			Noise & Duration & $N_{b}$ & RIAE & $\Delta_{\rm PSD}$ & $t$\,(s) & RIAE & $\Delta_{\rm PSD}$ & $t$\,(s) & RIAE & $\Delta_{\rm PSD}$ & $t$\,(s) \\
			\midrule
			\multirow{3}{*}{\texttt{4a}}
