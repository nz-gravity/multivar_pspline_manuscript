...
\begin{document}
	...
	$\phi_{j}$, $\nu_{j}$ are precision hyperparameters.
	\footnote{In the
	...
	\begin{figure}
		...
		\caption{Representative $3\times3$ spectral density matrix estimate for a
			...
			$N_{h}=4$ and $\eta=1$ (NUTS: $\mathrm{RIAE}=0.114$, 90\% CI
			coverage $88\%$; SVI: $\mathrm{RIAE}=0.102$, 90\% CI coverage $83\%$).
			Each panel shows the coarse-grained periodogram statistic
			$\Y_{h}^{*}/(T_{b}N_{b}N_{h})$ (grey), the theoretical spectrum
