...
\begin{document}
	...
	this section. Let
	$\Z=(\Z_{1},\ldots,\Z_{n})^{\top}\in \mathbb{R}^{n\times p}$ be a
	...
	autocovariance $\Gamma(\tau)=\mathbb{E}(\Z_{t}\Z_{t+\tau}^{\top})$ at
	...
	and $\left|\cdot\right|$ denotes the matrix determinant. In general a
	spectral density matrix is only guaranteed to be Hermitian positive
	semidefinite. The Whittle likelihood, however, requires
	$\S(f_{k})^{-1}$ to exist, so we assume strict positive definiteness
	throughout (a property the Cholesky parametrisation of
	Section~\ref{subsec:cholesky} enforces by construction).
	...
	\begin{align}
		...
		\codelink{https://github.com/nz-gravity/LogPSplinePSD/blob/v0.1.0/src/log_psplines/datatypes/multivar_utils.py\#L213}
	\end{align}
	...
	\begin{equation}
		...
		\codelink{https://github.com/nz-gravity/LogPSplinePSD/blob/v0.1.0/src/log_psplines/datatypes/multivar.py\#L248}
	\end{equation}
	...
	\begin{equation}
		...
		\codelink{https://github.com/nz-gravity/LogPSplinePSD/blob/v0.1.0/src/log_psplines/preprocessing/coarse_grain.py\#L177}
	\end{equation}
	...
	\begin{align}
		...
		\left|\S(\bar{f}_{h})\right|^{-N_{b}N_{h}}\times \nonumber \\
		                         ...
		                         \codelink{https://github.com/nz-gravity/LogPSplinePSD/blob/v0.1.0/src/log_psplines/pipeline/models.py\#L197}
	\end{align}
	...
	\begin{equation}
		...
		\codelink{https://github.com/nz-gravity/LogPSplinePSD/blob/v0.1.0/src/log_psplines/pipeline/models.py\#L241}
	\end{equation}
	...
	\begin{equation}
		...
		\exp\!\left(\frac{-\sum_{\nu=1}^{p}\left|u_{j\nu}^{(h)}-\sum_{l=1}^{j-1}\theta_{jl}^{(h)}\,u_{l\nu}^{(h)}\right|^{2}}
		              {T_{b}\,\delta_{jh}^{2}}\right), \codelink{https://github.com/nz-gravity/LogPSplinePSD/blob/v0.1.0/src/log_psplines/pipeline/models.py\#L81}
	\end{equation}
	\end{widetext}%
	...
	\begin{equation}
		...
		\codelink{https://github.com/nz-gravity/LogPSplinePSD/blob/v0.1.0/src/log_psplines/pipeline/models.py\#L244}
	\end{equation}
	...
	\begin{align}
		...
		\Im[\theta_{jl}(f_{k})] & = \sum_{m=1}^{K_{jl}}B_{m}(f_{k}) w_{jl,m}^{(\Im)}\label{eq:theta_im_spline} \codelink{https://github.com/nz-gravity/LogPSplinePSD/blob/v0.1.0/src/log_psplines/psplines/psplines.py\#L545}
	\end{align}
	...
	Knots are placed adaptively, in the spirit of
	\citet{maturana-russel_bayesian_2021}: for each Cholesky component the
	empirical periodogram is denoised, the absolute gradient of the result
	--- plus a small uniform floor so that featureless regions still
	receive knots --- is normalised to a probability mass function over
	frequency, and knots are positioned at equally spaced quantiles of its
	cumulative
	distribution.\codelink{https://github.com/nz-gravity/LogPSplinePSD/blob/v0.1.0/src/log_psplines/psplines/knots_locator/knot_locator.py\#L217}
	For the off-diagonal entries the real and imaginary parts are treated as
	...
	\begin{equation}
		...
		\codelink{https://github.com/nz-gravity/LogPSplinePSD/blob/v0.1.0/src/log_psplines/psplines/initialisation.py\#L150}
	\end{equation}
	...
	\begin{align}
		...
		\codelink{https://github.com/nz-gravity/LogPSplinePSD/blob/v0.1.0/src/log_psplines/pipeline/models.py\#L16}
	\end{align}
	...
	$10^{-3}$ for 2000
	iterations.\codelink{https://github.com/nz-gravity/LogPSplinePSD/blob/v0.1.0/src/log_psplines/pipeline/vi.py\#L134}
	Four NUTS chains are then initialised
	...
	\begin{equation}
		...
		\, \mathrm{d}f}{\int \! \left\lVert S(f) \right\rVert_{F}^{2} \, \mathrm{d}f}, \codelink{https://github.com/nz-gravity/LogPSplinePSD/blob/v0.1.0/src/log_psplines/diagnostics/_utils.py\#L56}
	\end{equation}
	...
	$\alpha=0.9$
	throughout.\codelink{https://github.com/nz-gravity/LogPSplinePSD/blob/v0.1.0/src/log_psplines/diagnostics/_utils.py\#L131}
	...
	\begin{align}
		...
		\codelink{https://github.com/nz-gravity/LogPSplinePSD/blob/v0.1.0/src/log_psplines/example_datasets/varma_data.py\#L379}
	\end{align}
	...
	The in-text code links (\scalebox{0.65}{\GitHubIcon}) are pinned to the tagged release \texttt{v0.1.0} of the package, so that the referenced line numbers remain valid as the codebase evolves.
