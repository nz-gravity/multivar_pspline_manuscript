...
\begin{document}
	...
	\begin{abstract}
		...
		achieves low relative mean squared (Frobenius) error (RMSE) that improves with
		sample size. We
		...
		LISA noise by an order of magnitude in relative mean squared
		(Frobenius) error.
	\end{abstract}
    ...
    relative mean squared (Frobenius) error while the full multivariate model
	...
	metrics. The first is the relative mean squared (Frobenius) error
	(RMSE),
	\begin{equation}
		\operatorname{RMSE}= \frac{\int \! \left\lVert \widehat{S}(f) - S(f) \right\rVert_{F}^{2}
	...
	posterior medians track each other closely ($\mathrm{RMSE}=0.114$
	...
	every configuration, and the mean RMSE varies by less than 3\% across
	...
	\begin{table*}[!htbp]
		...
		RMSE is the relative mean squared (Frobenius) error. ESS
    ...
    \Body
        $N_{b}$ & $N_{h}$ & $N_{c}$ & Coverage & RMSE & ESS & Runtime (s) \\
		\midrule
	...
	\begin{figure}
		...
		\caption{Representative $3\times3$ spectral density matrix estimate for a
			...
			$N_{h}=4$ and $\eta=1$ (NUTS: $\mathrm{RMSE}=0.114$, 90\% CI
			coverage $88\%$; SVI: $\mathrm{RMSE}=0.102$, 90\% CI coverage $83\%$).
	...
	\begin{itemize}
		...
		$S_{AA}, S_{EE}, S_{TT}$ of $\Y_{h}^{*}$ --- so the
		...
		knots on each off-diagonal component $\theta_{jl}$, forcing
	...
	$\mathrm{RMSE}(\mathcal{H}_{0})\!>\!\mathrm{RMSE}(\mathcal{H}_{1})\!>\!\mathrm{RMSE}(\mathcal{H}_{2})$
	for \texttt{noise5a} and
	$\mathrm{RMSE}(\mathcal{H}_{0})\!\approx\!\mathrm{RMSE}(\mathcal{H}_{1})\!\approx\!\mathrm{RMSE}(\mathcal{H}_{2})$
	for \texttt{noise4a}.
	...
	In addition to the RMSE introduced in Section~\ref{subsec:sim_3d_var2},
	...
	\end{equation}
	computed in the \XYZ basis for all three models. This complements the RMSE
	(a bias metric) with a summary of posterior precision.
	Table~\ref{tab:lisa_results} reports RMSE, $\Delta_{\mathrm{PSD}}$, and
	...
	($\mathrm{RMSE}\approx 4.4\!-\!5.3\times10^{-3}$) across all
	...
	the \AET off-diagonals, as the order-of-magnitude RMSE gap to
	...
	\begin{table*}[!htbp]
		...
		\caption{LISA TDI \XYZ results at fixed safe-Bayes
			...
			$1024$. RMSE is the relative mean squared (Frobenius)
		...
		\Body
			...
			Noise & Duration & $N_{b}$ & RMSE & $\Delta_{\rm PSD}$ & $t$\,(s) & RMSE & $\Delta_{\rm PSD}$ & $t$\,(s) & RMSE & $\Delta_{\rm PSD}$ & $t$\,(s) \\
			\midrule
			\multirow{3}{*}{\texttt{4a}}
   ...
   ~\citet{yixuan_vnpc}. Entries are mean $\pm$ standard deviation over 500
	...
	\begin{figure}[!t]
		...
		\caption{Safe-Bayes $\eta$-sensitivity on the symmetric LISA
			...
			Bottom: matrix RMSE (narrow $y$-range---RMSE varies by
	...
	First, the matrix RMSE is essentially flat across the entire sweep:
