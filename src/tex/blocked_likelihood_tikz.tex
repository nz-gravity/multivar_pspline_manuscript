...
%
% NOTE: ms.tex does NOT \input this file directly. The figure is included
% as a pre-rendered PDF (figures/blocked_likelihood.pdf) to keep Overleaf
% compile times under the free-tier budget. To rebuild the PDF after
% editing this file, run from src/tex/:
%     tectonic build_blocked_likelihood.tex
%     mv build_blocked_likelihood.pdf figures/blocked_likelihood.pdf
% See build_blocked_likelihood.tex for the standalone wrapper.
%
...
\node[block, right=22mm of Z.east, yshift=10.4mm] (b1)    {$b_1$};
...
% Arrow stage1 -> stage2 (aligned vertically with the other inter-stage arrows)
\draw[->, thick]
    ($(b1.east)!0.5!(bNb.east) + (2mm, 0)$) --
    ($(p1.west)!0.5!(pNb.west) + (-2mm, 0)$)
...
\node[stagelabel] at ($(avgcenter) + (0, 8mm)$)
    {Blocked periodogram $\mathbf{Y}(f_{k})$};
...
\node[stagelabel] at ($(cg1)!0.5!(cg4) + (0, -7mm)$)
    {Coarse-grained periodogram $\mathbf{Y}_{h}^{\mathrm{cg}}$};
...
\node[formulalabel, anchor=west, align=left]
    at ($(avg8.east) + (5mm, -8mm)$)
    {sum every $N_{h}$ frequencies\\[1pt]
     $\mathbf{Y}_{h}^{\mathrm{cg}}=\!\!\displaystyle\sum_{f_{k}\in J_{h}}\!\!\mathbf{Y}(f_{k})$};
